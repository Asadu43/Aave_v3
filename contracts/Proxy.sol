...
contract Proxy is IProxy, Storage, Config {
  ...
  function _setPostProcess(address to_) internal {
    ...
    if (stack.length == 0) {
      return;
    } else if (stack.peek() == bytes32(bytes12(uint96(HandlerType.Custom))) && bytes4(stack.peek(1)) != 0x00000000) {
      stack.pop();
  ...
  function _postProcess() internal {
    ...
    while (stack.length > 0) {
      ...
      if (handlerType == HandlerType.Token) {
        ...
        if (tokenAmount > 0) IERC20(addr).transfer(msg.sender, tokenAmount);
